...
\textsl{Melee Weapon Attack:} +4 to hit, reach 5 ft., one target. \textit{Hit:} 6 (\DndDice{1d8 + 2}) piercing damage. If the target is a humanoid, it must succeed on a DC 12 Constitution saving throw or be cursed with werewolf lycanthropy.

\DndMonsterAction{Claws (Hybrid Form Only)}
\textsl{Melee Weapon Attack:} +4 to hit, reach 5 ft., one creature. \textit{Hit:} 7 (\DndDice{2d4 + 2}) slashing damage.

\DndMonsterAction{Spear (Humanoid Form Only)}
\textsl{Melee or Ranged Weapon Attack:} +4 to hit, reach 5 ft. or range 20/60 ft., one creature. \textit{Hit:} 5 (\DndDice{1d6 + 2}) piercing damage, or 6 (\DndDice{1d8 + 2}) piercing damage if used with two hands to make a melee attack.
